	...
	public JSONArray getGwasListJson(String permissionUid, String varietyid) throws Exception
	{
		 ...
		 JSONArray jsonArray = new JSONArray();
			...
			try{
				...
				String sql="select no, cropid, filename, status, uploadpath, resultpath, comment, jobid, DATE_FORMAT(cre_dt, '%Y-%m-%d') AS cre_dt  from gwas_info_t where creuser='"+permissionUid+"' and varietyid='"+varietyid+"' order by no desc;";
