...
<%
	...
	try{
		...
		/*
		String sql = "(select 'GWAS' as category, genotype_filename as filename, comment, jobid, cre_dt from gwas_info_t where creuser='" +permissionUid+ "' and varietyid = '" +varietyid+ "' and status=1"; 
		if(!year.equals("-1")) {
			sql += " and year(cre_dt) = '" +year+ "'";
		} 
		if(!month.equals("-1")) {
			sql += " and month(cre_dt) = '" +month+ "'";
		}
		sql += ")";
		
		
		sql += " UNION (select 'genotype_filter_t' as category, filename, '-' as comment, jobid, cre_dt from genotype_filter_t where creuser='" +permissionUid+ "' and varietyid = '" +varietyid+ "' and status=1";
		if(!year.equals("-1")) {
			sql += " and year(cre_dt) = '" +year+ "'";
		} 
		if(!month.equals("-1")) {
		...
		sql += " UNION (select 'Core selection' as category, filename, comment, jobid, cre_dt from genocore_info_t where creuser='" +permissionUid+ "' and varietyid = '" +varietyid+ "' and status=1";
		if(!year.equals("-1")) {
			sql += " and year(cre_dt) = '" +year+ "'";
		} 
		if(!month.equals("-1")) {
		...
		sql += " UNION (select 'Minimal marker' as category, filename, comment, jobid, cre_dt from mini_info_t where creuser='" +permissionUid+ "' and varietyid = '" +varietyid+ "' and status=1";
		if(!year.equals("-1")) {
			sql += " and year(cre_dt) = '" +year+ "'";
		}
		if(!month.equals("-1")) {
		...
		sql += " UNION (select 'PCA' as category, filename, comment, jobid, cre_dt from pca_info_t where creuser='" +permissionUid+ "' and varietyid = '" +varietyid+ "' and status=1";
		if(!year.equals("-1")) {
			sql += " and year(cre_dt) = '" +year+ "'";
		}
		if(!month.equals("-1")) {
		...
		sql += " UNION (select 'UPGMA clustering' as category, filename, comment, jobid, cre_dt from upgma_info_t where creuser='" +permissionUid+ "' and varietyid = '" +varietyid+ "' and status=1";
		if(!year.equals("-1")) {
			sql += " and year(cre_dt) = '" +year+ "'";
		} 
		if(!month.equals("-1")) {
